...
holedatadir = os.getcwd() + 'HoleData/'
logmessages = []
'''

fig = plt.figure(num=1, figsize=(7,9))
ex = fig.add_subplot(2, 3, (1, 2))
ftl = fig.add_subplot(2, 3, (3, 6))
pr_ts = fig.add_subplot(2, 3, 4)
speed = fig.add_subplot(2, 3, 5)
fig.subplots_adjust(top=0.85)

# Set titles for the figure and the subplot respectively
fig.suptitle('SAMPLE DRILL REPORT', fontsize=14, fontweight='bold')
ex.set_title('axes title')

ftl.set_title('force/torque/lith')
ftl.axis([0, 280, 8000, 0])
pr_ts.set_title('pitch/roll/ts')
pr_ts.axis([-30, 30, 12000, 0])
speed.set_title('penrate')
speed.axis([0, 500, 8000, 0])


'''
ex.set_xlabel('xlabel')
ex.set_ylabel('ylabel')
# Set both x- and y-axis limits to [0, 10] instead of default [0, 1]
ex.axis([0, 10, 0, 10])

ex.text(3, 8, 'boxed italics text in data coords', style='italic',
        bbox={'facecolor': 'red', 'alpha': 0.5, 'pad': 10})
ex.text(2, 6, r'an equation: $E=mc^2$', fontsize=15)
ex.text(3, 2, 'unicode: Institut für Festkörperphysik')
ex.text(0.95, 0.01, 'colored text in axes coords',
        verticalalignment='bottom', horizontalalignment='right',
        transform=ex.transAxes,
        color='green', fontsize=15)
ex.plot([2], [1], 'o')
ex.annotate('annotate', xy=(2, 1), xytext=(3, 4),
...
'''
def check_dir(holedatadir):
# --- check all files in directory
    tooltrendlist = []
    for file in os.listdir(holedatadir):
...
def create_plot(ttfile, template):
    ...
    try:
        ...
        return f'\nIgnored {ttfile}: NOT IN SCREENLOG\n\n\n'
    sampleid = data['Sample_ID']
    if data['EOH_Code'] == 'ABD':
        return f'\nSkipped {sampleid}: ABANDONED\n\n\n'
    units = []
    lines = []
...
with open(imdh, 'r') as heading:
    print('\n', heading.read())
print('\nTTPLOT SAMPLE TOOLTREND PLOT version 1.2.6')
print('Reading files . . .')
screenlogfile, tooltrendlist = check_dir(os.getcwd())
print(f'{len(tooltrendlist)} Hole Data Files found')
print('Reading screenlog . . .')
screenlog = read_screenlog(screenlogfile)
for n, file in enumerate(tooltrendlist):
    print(f'\nPlotting {file} [{n+1}/{len(tooltrendlist)}]\n')
    template = template_layout()
    try:
        msg = create_plot(file, template)
        print(msg)
    except Exception as e:
        print(e)
