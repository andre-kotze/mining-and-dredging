...
class Worker(QObject):
    ...
    def readfile(self):
        ...
        QuasarExport.sldf = read_sl(self.arglist)
        self.done.emit()
...
class QuasarUI(QWidget):
    ...
    def initQW(self):
        ...
        self.abouttext = QLabel(
            ...
            Written 2021 by Andre "Andrew" "Fucking Legend" "Drikus" "Kotez" Kotze\n
...
class QuasarExport(QWidget): #TableWidget
    ...
    def initQE(self):
        ...
        self.sldf, self.savedfolder = None, None
    ...
    def export(self, file, last, cols, ftype):
        ...
        if outdest is None:
            ...
            count = 0
            while count < est_read and not self.read_prog.wasCanceled():
                count += 1
                time.sleep(0.5)
            ...
            if QuasarExport.sldf is None:
                ...
                if ftype == 'csv':
                    ...
                    self.worker = Worker([QuasarExport.sldf, outdest, filename]) # no parent, so it can be moved
                    self.external = QThread(self)
                    self.worker.moveToThread(self.external)
                    ...
                    self.external.start()
                    count = 0
                    while count < est_toshp and not self.write_prog.wasCanceled():
                        count += 1
