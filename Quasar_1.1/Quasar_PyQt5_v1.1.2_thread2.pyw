...
class Worker(QObject):
    ...
    def readfile(self):
        ...
        QuasarExport.sldf = read_sl([QuasarExport.lddsl, None, True, 'Sample_ID'])
        self.done.emit()
        print(f'QuasarExport.sldf:\ntype: {type(QuasarExport.sldf)}\nlen: {len(QuasarExport.sldf)}')
        print("done")

    def exportshpfile(self, saveargs):
        print('Worker start exporting')
        report, folder  = exportshp(saveargs)
        self.done.emit()
        print('done')
...
class QuasarExport(QWidget): #TableWidget
    ...
    def lookbusy(self, ftype):
        ...
        self.export(self.lddsl, self.recgroup.checkedId(), self.colgroup.checkedId(), ftype)
    ...
    def export(self, file, last, cols, ftype):
        ...
        est_read = int(dur * 10)
        print(f'est_read = {dur} * 10 = {est_read}ds')
        est_toshp = int(dur * 50)
        print(f'est_toshp = {dur} * 50 = {est_toshp}ds')
        ...
        if self.setdirs != 'True':
            ...
            if ftype == 'csv':
                outdest = self.csvdest
            elif ftype == 'shp':
                outdest = self.shpdest
        if outdest is None:
            report, folder = 'Cancelled', None
        else:
            self.worker = Worker() # no parent, so it can be moved
            ...
            count = 0
            while count < 70 and not self.read_prog.wasCanceled():
                count += 1
                time.sleep(0.1)
            ...
            print(' thread exit')
            self.external.exit()
            print(type(QuasarExport.sldf))
            if QuasarExport.sldf is None:
                print('empty df received')
                report, folder = 'SL Read Error\n(╯°□°)╯͡ ┻━┻\n', None
            else:
                # EXPORT FILE:
                if ftype == 'csv':
                    report, folder = exportcsv(QuasarExport.sldf, outdest, filename)
                elif ftype == 'shp':
                    report, folder = exportshp([QuasarExport.sldf, outdest, filename])
                else:
...
def exportshp(saveargs):
    ...
    if os.path.exists(f'{outdest}/{filename}.shp') and QuasarExport.setdirs == 'True':
        ...
        while os.path.exists(f'{outdest}/{filename}_{n}.shp'):
            ...
            newname = f'{filename}_{n}'
        filename = newname
    print(f'shp export: {dt.datetime.now()}')
    try:
        ...
        print(e1)
        print(f'shp1 failed: {dt.datetime.now()}\nshp2 . . .')
        try:
