...
class GoingDeepUI(QWidget):
    ...
    def create_recon(self):
        ...
        self.crlbbtn.setFixedSize(22, 22)
        self.readcrl_btn = QPushButton('Load', self)
    ...
    def create_check(self):
        ...
        self.checkdirbtn.setFixedSize(22, 22)
    ...
    def import_sl(self):
        if not os.path.exists(self.lddsl):
            print('import failed: no such file')
            return
    ...
    def import_crl(self):
        if not os.path.exists(self.lddcrl):
            print('import failed: no such file')
            return
        self.readcrl_btn.setEnabled(False)
        self.readcrl_btn.setText('Loading')
        QApplication.processEvents()
        ...
        if self.crldf is not None:
            ...
            self.readcrl_btn.setText('Reload')
        else:
            print('import failed')
            self.readcrl_btn.setText('Load')
        self.readcrl_btn.setEnabled(True)
    ...
    def reconcile(self):
        ...
        if self.crldf is None:
            self.crldf = read_crl(self.lddcrl, filterdate)
        sldf_trim = self.sldf[self.sldf['Start_Date'] == filterdate]
    
        compare_tables(sldf_trim, self.crldf)
        sldf_00check = self.sldf[self.sldf['End_Date'] == filterdate]
        if sldf_trim.Sample_ID.tolist() != sldf_00check.Sample_ID.tolist():
            midnight_check(sldf_00check, filterdate)
...
def read_crl(filename, filterdate):
    ...
    try:
        ...
        dur = dur.seconds + (dur.microseconds / 10**6)
...
def compare_tables(sldf, crldf):
    ...
    if len(elist) != 0:
        ...
        report.setDetailedText('\n'.join(elist))
        report.setStyleSheet("QTextEdit{min-width: 300px; min-height: 200px}")
    else:
        report.setText('Times match')
    report.setWindowIcon(QIcon('icon.ico'))
    report.exec()

def check_trend(hddir, f):
    try:
        ttdata = pd.read_excel(hddir + f, skiprows=1, usecols=['Time\nhh:mm:ss', 'Drill Depth\nmm'])
