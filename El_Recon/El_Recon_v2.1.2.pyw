...
class El_Recon_Dos(QWidget):
    ...
    def recon_drs_sl(self):
        ...
        content = {
            'title' : 'Geological Screen Log vs Diamond Record Sheet', #str
    ...
    def recon_sl_sr(self):
        ...
        el_sldf.Colour = el_sldf.Colour.apply(listify_colcla)
        el_sldf.Clarity = el_sldf.Clarity.apply(listify_colcla)
        unsorted_abd = el_sldf[el_sldf.EOH_Code == 'ABD'].index.tolist()
        unsorted_abt = el_sldf[el_sldf.EOH_Code == 'ABT'].index.tolist()
        ...
        for sid, sample in el_sldf.iterrows():
            ...
            try:
                ...
                if sid in unsorted_abd:
                    info = ' [ABD]'
                elif sid in unsorted_abt:
                    info = ' [ABT]'
                ind_err.append(f'Sample not found: {str(k)} {info}')
            except Exception as e:
                ind_err.append(f'Err: {type(e)}: {str(e)}')
            if len(ind_err) > 0:
                report.append(f'\n{sid}')
        ...
        content = {
            'title' : 'Geological Screen Log vs Sampling Results Sheet', #str
            'dates' : f'{inferred_conc}: {dates[0]} to {dates[1]}', #str
            'refs' : refs, #list
            'report' : report, #list
            'num' : (self.report_nr + 1)
