...
class Los_Errors(QMessageBox):
    ...
    def resizeEvent(self, event):
        result = super(Los_Errors, self).resizeEvent(event)
        details = self.findChild(QTextEdit)
        if details is not None:
            details.setFixedSize(800, 600)
        return result
    
class El_Informe(QWidget):
    def __init__(self, title='REPORT', text='Recon completed', reportlist=None):
        super(QWidget, self).__init__()
        self.setWindowTitle(title)
        self.setWindowIcon(QIcon('icon.ico'))
        self.resize(800, 600)
        self.report = QTextEdit(reportlist)
        self.ok = QPushButton('OK')
        self.ok.clicked.connect(self.close)
        
        layout = QGridLayout()
        layout.addWidget(self.report,0,0,1,2)
        layout.addWidget(self.ok,1,2,1,1,)
...
class El_Recon_Dos(QWidget):
    ...
    def recon_drs_sl(self):
        ...
        if self.drsdf is None:
            edf = Los_Errors('Import DRS', 'No DRS loaded')
            edf.exec()
            return
        if self.sldf is None:
            edf = Los_Errors('Import Screenlog', 'No Screenlog loaded')
            edf.exec()
            return
        #filter drsdf:
        ...
        for sid, sample in el_sldf.iterrows():
            ...
            if len(ind_err) > 0:
                ...
                mmcount += 1
        if mmcount == 0:
            report.append('100% match')
        else:
            heading.insert(-1, '***Screenlog value shown first')
            report.insert(0, f'{mmcount} total mismatches')
            report.append('\n\n[end of report]')
        final = Los_Errors('REPORT','Recon finished', heading + report)
        final.exec()
        return

    def recon_sl_sr(self):
        report = ['EL RECON: SCREENLOG vs SAMPLING RESULTS'] # report 0
        dates = [self.start_date.date().toString("yyyy-MM-dd"), self.end_date.date().toString("yyyy-MM-dd")]
        report.append(f'{dates[0]} to {dates[1]}\n\n') # report 1
        # update last date in config
        config.set('last_used', 'start', dates[0])
        config.set('last_used', 'end', dates[1])
        with open('config.ini', 'w') as f:
        ...
        if self.srdf is None:
            edf = Los_Errors('Import SamplingResults', 'No SR sheet loaded')
            edf.exec()
            return
        # filter sl:
        el_sldf = self.sldf[self.sldf['Start_Date'].between(dates[0], dates[1])]
