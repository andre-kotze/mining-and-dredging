...
def read_drs(name, dates):
    ...
    #try:
    drs = pd.read_excel(name, sheet_name='3C_DiamondSortingLogs', skiprows=1, usecols='A,C,D,H,AM:AO,AV:AOM')
...
def compare_series(series1, series2, label, ids): # ids is a series of SampleIDs
    if series1.eq(series2).any() == False:
        ...
        if len(mm) == 0:
            ...
            for m in mm:
                print('Check SampleID:\t', ids.loc[m])
    else:
...
def sl_vs_drs(sl, drs, max_stones):
    ...
    for m in compare_lists(collec_cts, sl['Est_Brkdwn'].values.tolist(),'Carat'):
        print('Check Seq:\t', drs.loc[m, 'Seq No.'], 'SampleID:\t', drs.loc[m, 'Sample ID'])
    for m in compare_lists(collec_shps, sl['Shape'].values.tolist(),'Shape'):
        print('Check Seq:\t', drs.loc[m, 'Seq No.'], 'SampleID:\t', drs.loc[m, 'Sample ID'])
    for m in compare_lists(collec_cols, sl['Colour'].values.tolist(),'Colour'):
        print('Check Seq:\t', drs.loc[m, 'Seq No.'], 'SampleID:\t', drs.loc[m, 'Sample ID'])
    for m in compare_lists(collec_cla, sl['Clarity'].values.tolist(),'Clarity'):
        print('Check Seq:\t', drs.loc[m, 'Seq No.'], 'SampleID:\t', drs.loc[m, 'Sample ID'])
...
def main():
    ...
    #if (drs == None) or (sl == None) or (sr == None):
    #    print('Check the input sheets and try again')
    #    sys.exit(0)

    # convert sl_by_sample to screenlog_by_stone
