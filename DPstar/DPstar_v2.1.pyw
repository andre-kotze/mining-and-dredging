...
class DPMessage(QMessageBox):
    def __init__(self, title='DP⛧', msg=''):
        super().__init__()
...
class DPStarUI(QWidget):
    ...
    def selectfile(self):
        file = QFileDialog.getOpenFileName(None, 'Open Drilling Program xlsx', os.path.dirname(self.last_dp), 'Drilling Program (*.xlsx)')[0]
    ...
    def export(self):
        ...
        if self.gsl_opt.isChecked() or self.dsl_opt.isChecked():
            ...
            if not os.path.exists(folder):
                return
            busy = DPMessage(msg='Reading DP...').show()
            ...
            if self.dsl_opt.isChecked():
                ...
                if os.path.exists(dsl_fname):
                    dsl_fname = rename(dsl_fname)
        ...
        # message report:
        msg = DPMessage('Export Data Source', '\n'.join(report))
...
def rename(fullname): # rename to avoid overwrite
    c = 1
    newname = fullname
    while os.path.exists(newname):
        newname = f'{fullname[:-4]}({c}){fullname[-4:]}'
        c += 1
    return newname
...
def main():
    ...
    # Show the GUI
    dpui = QMainWindow()
    dpui.setWindowTitle('DP⛧ DrillingProgram Star v2.1 by AKO')
